 ...
 *
 * Texnik jihati umumiy `Marquee` komponentida: kontent ikki marta chiziladi
 * va lenta -50% ga suriladi, faqat `transform` animatsiya qilinadi — JS
 * umuman ishlamaydi. `prefers-reduced-motion` da animatsiya to'xtaydi
 * (globals.css) va tasma oddiy gorizontal ro'yxatga aylanadi.
 */
export function PriceTicker({ tours, locale }: Props) {
  ...
  return (
    <div
      className="relative border-y border-line bg-navy-950 py-3"
    ...
    >
      ...
      {/* Har bir element ~5 soniyada o'tadi */}
      <Marquee durationSeconds={items.length * 5} itemsClassName="items-center gap-8 px-4">
        {items.map((tour, index) => {
          ...
          return (
            <span key={`${tour.id}-${index}`} className="flex shrink-0 items-center gap-2.5">
